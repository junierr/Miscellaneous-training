...
class MarchingCubesVisualizer:
    ...
    def __init__(self, screenWidth=800, screenHeight=600, worldSize=10, debug=True, radius=20):
        ...
        self.world = [[[0 for x in range(self.worldSize)] for y in range(self.worldSize)] for z in
                      range(self.worldSize)]
        self.worldThreshold = 0.5
        self.calculateWorldValues()

        # perform initial marching cubes
        self.polygons = [[[[] for x in range(self.worldSize)] for y in range(self.worldSize)] for z in
                         range(self.worldSize)]
        self.findPolygons()
            ...
            for y in range(self.worldSize):
                for z in range(self.worldSize):
                    if self.t=='b':
                        ...
                        self.world[x][y][z] = math.sin(x)*math.cos(y) +math.sin(z)*math.cos(x)+math.sin(y)*math.cos(z)
